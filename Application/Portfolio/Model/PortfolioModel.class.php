...
class PortfolioModel extends Model{
    ...
    public function editData($data)
    {
        ...
        if($data['id']){
            ...
            $res=$this->save($data);
            $detail['portfolio_id']=$data['id'];
        }else{
            $data['create_time']=$data['update_time']=time();
            $res=$this->add($data);
            action_log('add_portfolio', 'Portfolio', $res, is_login());
            $detail['portfolio_id']=$res;
        }
        if($res){
            D('Portfolio/PortfolioDetail')->editData($detail);
        }
        return $res;
    }
    ...
    **/
    ...
    public function getData($id)
    {
        if($id>0){
            ...
            if($data){
                $data['detail']=D('Portfolio/PortfolioDetail')->getData($id);
            }
            return $data;
        }
        return null;
    ...
    private function listMap($category, $status = 1, $pos = null){
        ...
        /* 设置分类 */
        if(!is_null($category)){
            $cates=D('Portfolio/PortfolioCategory')->getCategoryList(array('pid'=>$category,'status'=>1));
