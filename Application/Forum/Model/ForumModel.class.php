...
class ForumModel extends Model
{
    ...
    public function getForumList($map_type = array('status' => 1))
    {
        ...
        if (empty($forum_list)) {
            ...
            $forum_list = $this->where($map_type)->order('sort asc')->select();
            ...
            foreach ($forum_list as &$f) {
                ...
                $f['topic_count'] = D('ForumPost')->where($map)->count();
                $f['admin'] = explode(',', str_replace('[', '', str_replace(']', '', $f['admin'])));
                $post_id = D('ForumPost')->where(array('forum_id' => $f['id']))->field('id')->select();
                $p_id = getSubByKey($post_id, 'id');
                $map['post_id'] = array('in', implode(',', $p_id));
                $f['total_count'] = $f['topic_count'] + D('ForumPostReply')->where($map)->count();
            }
            unset($f);
