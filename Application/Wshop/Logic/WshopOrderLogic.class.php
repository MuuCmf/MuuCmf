...
namespace Wshop\Logic;
use Think\Model;
use Wshop\Model\WshopOrderModel as WshopOrderModel;
/*
 * 订单逻辑层
 */
class WshopOrderLogic extends Model{
	...
	function _initialize()
	{
		...
		$this->delivery_model = D('Wshop/WshoppDelivery');
		$this->coupon_logic = D('Wshop/WshopCoupon','Logic');
		$this->user_coupon_logic = D('Wshop/WshopUserCoupon','Logic');
		$this->cart_model         = D('Wshop/WshopCart');
		$this->product_comment_model = D('Wshop/WshopProductComment');
		$this->product_sell_model = D('Wshop/WshopProductSell');
	...
	{
		if ($order['status'] != WshopOrderModel::ORDER_WAIT_USER_PAY) {
			$this->error_str = '错误的订单状态';
			return false;
		}
		$update = array(
			'id' => $order['id'],
			'status' => WshopOrderModel::ORDER_WAIT_FOR_DELIVERY,
	...
	public function AfterPayOrder($data,$odata)
	{
		...
		if(!empty($shop_order))
		{
			...
			$shop_order['pay_type'] = WshopOrderModel::PAY_TYPE_WEIXINPAY;
	...
	{
		if (!in_array($order['status'], array(WshopOrderModel::ORDER_WAIT_USER_PAY,
		                                  //ShopOrderModel::ORDER_WAIT_FOR_DELIVERY, 待发货时可以申请退款，不要取消
		                                  WshopOrderModel::ORDER_UNDER_NEGOTATION,
		                                  WshopOrderModel::ORDER_WAIT_SHOP_ACCEPT))
		) {
			$this->error_str = '错误的订单状态';
			return false;
		}
		...
		$update = array(
			'id' => $order['id'],
			'status' => (!(CONTROLLER_NAME=='Index')?WshopOrderModel::ORDER_SHOP_CANCELED: WshopOrderModel::ORDER_CANCELED),
		);
		$this->startTrans();
...
//		$order = $this->order_model->get_order_by_id($order['id']);
	...
	public function send_good($order,$deliver_info){
		if ($order['status'] != WshopOrderModel::ORDER_WAIT_FOR_DELIVERY) {
//			setLastError(ERROR_BAD_STATUS);
		...
		$update = array(
			'id' => $order['id'],
			'status' => WshopOrderModel::ORDER_WAIT_USER_RECEIPT,
...
//			setLastError(ERROR_BAD_STATUS);
		...
		$update = array(
			'id' => $order['id'],
			'status' => WshopOrderModel::ORDER_DELIVERY_OK,
			'recv_time' => $_SERVER['REQUEST_TIME'],
		);
		$product_cell_model = D('Wshop/WshopProductSell');
		$this->startTrans();
		if(!$this->order_model->add_or_edit_order($update))
		{
	...
	public function negotation_order($order,$info = array())
	{
		$update = array(
			'id' => $order['id'],
			'status' => WshopOrderModel::ORDER_UNDER_NEGOTATION,
		);
		if ($info) {
			$update['info'] = $info;
		}
	...
	{
		if ($order['status'] != WshopOrderModel::ORDER_UNDER_NEGOTATION) {
			$this->error_str ='错误的订单状态';
			return false;
		}
		...
		$update = array(
			'id' => $order['id'],
			'status' => WshopOrderModel::ORDER_NEGOTATION_OK,
		);
		$this->order_model->add_or_edit_order($update);
		return true;
	}
	...
	public function delete_order($ids)
	{
		...
		if (!in_array($order['status'], array(WshopOrderModel::ORDER_CANCELED, WshopOrderModel::ORDER_WAIT_USER_PAY))) {
...
*/
	...
	*/
	...
	public  function add_product_comment($product_comments)
	{
		...
		if(!$this->order_model->where($may)->save(array('status' => WshopOrderModel::ORDER_COMMENT_OK)))
		{
	...
	public function BeforePayOrder($order_id,$user_id,$mp_id)
	{
		...
		if($shop_order['status']!=WshopOrderModel::ORDER_WAIT_USER_PAY || $shop_order['paid_fee'] ==0)
		{
